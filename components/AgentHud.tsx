...
const AgentHud: React.FC<Props> = ({ state, agents }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#09090b]">
      {/* Tabs */}
      <div className="flex-none flex items-center justify-between px-3 pt-2 border-b border-white/5 bg-[#121214]/50 backdrop-blur-xl">
        <div className="flex items-center gap-1 overflow-x-auto">
          {['live', 'audit', 'sources', 'brain'].map((tab) => (
             ...
             >
                 ...
                 {activeTab === tab && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[var(--accent-primary)] shadow-[0_0_8px_var(--accent-primary)]"></div>}
             </button>
          ))}
        </div>
        <div className="flex items-center gap-4 pr-3 pb-1">
            <span className="text-[9px] font-black text-zinc-500 uppercase bg-zinc-900 px-2 py-0.5 rounded border border-white/5">
      ...
      <div className="flex-1 overflow-hidden relative">
        ...
        {activeTab === 'live' && (
            <div className="h-full flex flex-col">
                <div className="h-[180px] md:h-[200px] flex-none border-b border-white/5 bg-[#09090b] flex items-center justify-center relative">
                ...
                <div className="flex-1 flex flex-col bg-[#09090b] text-zinc-400 font-mono text-[10px]">
                    <div className="px-4 py-2 border-b border-white/5 bg-[#121214] flex items-center justify-between">
                        <span className="font-black text-zinc-500 uppercase tracking-widest flex items-center gap-2 text-[9px]">
                            <IconCode className="w-3 h-3" />
                            NEURAL_LOG_UPLINK
                        </span>
                        <span className="text-[9px] font-mono text-zinc-600">STREAMS_ENCRYPTED</span>
                    </div>
                    <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                        ...
                        {state.logs.map((log, i) => {
                            ...
                            return (
                                <div key={i} className={`break-all border-l-2 pl-3 py-0.5 ${isFail ? 'border-red-500 bg-red-500/5' : isML ? 'border-purple-500 bg-purple-500/5' : isApi ? 'border-blue-500 bg-blue-500/5' : isDeploy ? 'border-emerald-500 bg-emerald-500/5' : 'border-zinc-800'}`}>
                                    <span className="text-zinc-600 mr-2">[{new Date().toLocaleTimeString().split(' ')[0]}]</span>
                                    <span className={isFail ? 'text-red-400 font-bold' : isML ? 'text-purple-400' : isApi ? 'text-blue-300' : isDeploy ? 'text-emerald-400' : 'text-zinc-300'}>
                                        {log.toUpperCase()}
                                    </span>
                                </div>
                            );
